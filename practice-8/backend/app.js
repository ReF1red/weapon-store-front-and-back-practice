...
const JWT_SECRET = process.env.JWT_SECRET || 'arms_store_secret_key_2026';
...
const swaggerOptions = {
  definition: {
    openapi: '3.0.0',
    info: {
      title: 'Arms Store API with JWT',
      version: '1.1.0',
      description: 'API для управления каталогом оружейного магазина с JWT авторизацией'
    },
    servers: [
      {
    ...
    paths: {
      '/api/auth/register': {
        post: {
          ...
          responses: { 201: { description: 'Пользователь создан' }, 400: { description: 'Ошибка валидации' }, 409: { description: 'Пользователь уже существует' } }
        }
      },
      '/api/auth/login': {
        post: {
          ...
          responses: { 200: { description: 'Успешный вход' }, 400: { description: 'Ошибка валидации' }, 401: { description: 'Неверные данные' } }
        }
      },
      '/api/auth/me': {
        get: {
          ...
          security: [{ bearerAuth: [] }],
          responses: { 200: { description: 'Данные пользователя' }, 401: { description: 'Нет/невалидный токен' } }
        }
      },
      '/api/products': {
        get: {
        ...
        post: {
          ...
          responses: { 201: { description: 'Товар создан' }, 400: { description: 'Ошибка валидации' }, 401: { description: 'Нет токена' } }
        }
      },
      '/api/products/{id}': {
        get: {
          ...
          responses: { 200: { description: 'Товар найден' }, 401: { description: 'Нет токена' }, 404: { description: 'Товар не найден' } }
        },
        put: {
          ...
          responses: { 200: { description: 'Товар обновлен' }, 400: { description: 'Ошибка валидации' }, 401: { description: 'Нет токена' }, 404: { description: 'Товар не найден' } }
        },
        delete: {
          summary: 'Удалить товар',
          security: [{ bearerAuth: [] }],
          parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }],
          responses: { 204: { description: 'Товар удален' }, 401: { description: 'Нет токена' }, 404: { description: 'Товар не найден' } }
        }
      }
    }
  },
